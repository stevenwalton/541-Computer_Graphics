...
OpenDatabase("/research/earthquake/sw4_out.cycle_001500.root", 0)
...
#   VolumeAtts.gradientType = VolumeAtts.SobelOperator  # CenteredDifferences, SobelOperator
#   VolumeAtts.num3DSlices = 1000
#   VolumeAtts.scaling = VolumeAtts.Linear  # Linear, Log, Skew
#   VolumeAtts.skewFactor = 1
#   VolumeAtts.limitsMode = VolumeAtts.OriginalData  # OriginalData, CurrentPlot
#   VolumeAtts.sampling = VolumeAtts.Rasterization  # KernelBased, Rasterization, Trilinear
#   VolumeAtts.rendererSamples = 5
#   #transferFunction2DWidgets does not contain any TransferFunctionWidget objects.
#   VolumeAtts.transferFunctionDim = 1
#   VolumeAtts.lowGradientLightingReduction = VolumeAtts.Lower  # Off, Lowest, Lower, Low, Medium, High, Higher, Highest
#   VolumeAtts.lowGradientLightingClampFlag = 0
...
swatts.height = 1024
file_index = 0
base_fName = "/home/users/swalton2/Pictures/rotate/frame_"
for theta in np.arange(0,2,0.01):
    #for phi in np.arange(-1,1,0.1):
    View3DAtts.viewNormal = (theta,0.5,-0.3)
    angle = "{:0.2f}".format(theta)
    SetView3D(View3DAtts)
    swatts.fileName = base_fName + str(15) + "_" + angle + ".png"
    SetSaveWindowAttributes(swatts)
    SaveWindow()

#for theta in np.arange(1,-1,0.01):
#    phi = theta
#    #for phi in np.arange(-1,1,0.1):
#    View3DAtts.viewNormal = (theta,phi,-0.3)
#    angle = "{:0.2f}_{:0.2f}".format(theta,phi)
#    SetView3D(View3DAtts)
#    swatts.fileName = base_fName + str(11) + "_" + angle + "_reversed.png"
#    SetSaveWindowAttributes(swatts)
#    SaveWindow()
...
#    dbAtts.db_type = "VTK"
#    dbAtts.filename = "/research/earthquake/visit_data/raw/" + time
#    dbAtts.variables = ("default", "something_magnitude")
